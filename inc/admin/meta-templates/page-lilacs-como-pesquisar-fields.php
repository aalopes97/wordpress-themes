...
// Constantes para configurações de FAQ
const BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES = '_lilacs_cp_faq_selected_categories';
const BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER = '_lilacs_cp_faq_display_order';
const BIREME_LILACS_CP_META_FAQ_SHOW_EMPTY = '_lilacs_cp_faq_show_empty_categories';
...
function bireme_lilacs_cp_render_metabox($post){
  ...
  $selected_categories = get_post_meta($post->ID, BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES, true);
  $selected_categories = is_array($selected_categories) ? $selected_categories : [];
  $display_order = get_post_meta($post->ID, BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER, true) ?: 'name_asc';
  $show_empty = get_post_meta($post->ID, BIREME_LILACS_CP_META_FAQ_SHOW_EMPTY, true);

  // Busca todas as categorias de FAQ disponíveis
  ...
  <style>
    ...
    /* Estilos para configurações de FAQ */
    .faq-config-section{padding:12px 0;}
    .faq-config-section .checkbox-list{max-height:200px;overflow-y:auto;border:1px solid #ddd;padding:12px;background:#f9f9f9;border-radius:4px;}
    .faq-config-section .checkbox-list label{font-weight:normal;display:flex;align-items:center;margin:6px 0;}
    .faq-config-section .checkbox-list input[type=checkbox]{margin-right:8px;}
    .faq-config-section select{width:100%;max-width:400px;}
  </style>
  ...
  <div class="lilacs-tabs" data-tabs>
    <div class="lilacs-tabs__nav">
      ...
      <button type="button" class="lilacs-tabs__btn" data-tab-target="#tab-faq-config"><?php _e('Configurações FAQ', 'bireme'); ?></button>
    </div>
    ...
    <!-- Nova aba: Configurações de FAQ -->
    <div id="tab-faq-config" class="lilacs-tab">
      <div class="faq-config-section">
        <div class="field">
          <label><?php _e('Categorias de FAQ para exibir', 'bireme'); ?></label>
          <div class="checkbox-list">
            <?php if (!empty($faq_categories) && !is_wp_error($faq_categories)): ?>
              <label>
                <input type="checkbox" id="select_all_categories" onchange="toggleAllCategories(this)">
                <strong><?php _e('Selecionar todas', 'bireme'); ?></strong>
              </label>
              <hr style="margin:8px 0;">
              <?php foreach ($faq_categories as $category): ?>
                <label>
                  <input type="checkbox" 
                         name="<?php echo BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES; ?>[]" 
                         value="<?php echo esc_attr($category->term_id); ?>"
                         <?php checked(in_array($category->term_id, $selected_categories) || empty($selected_categories)); ?>>
                  <?php echo esc_html($category->name); ?> 
                  <small>(<?php echo $category->count; ?> <?php _e('perguntas', 'bireme'); ?>)</small>
                </label>
              <?php endforeach; ?>
            <?php else: ?>
              <p><?php _e('Nenhuma categoria de FAQ encontrada.', 'bireme'); ?></p>
            <?php endif; ?>
          </div>
          <p class="lilacs-help"><?php _e('Selecione quais categorias devem aparecer na página. Se nenhuma for selecionada, todas aparecerão.', 'bireme'); ?></p>
        </div>

        <div class="field">
          <label for="<?php echo BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER; ?>"><?php _e('Ordem de exibição das categorias', 'bireme'); ?></label>
          <select name="<?php echo BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER; ?>" id="<?php echo BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER; ?>">
            <option value="name_asc" <?php selected($display_order, 'name_asc'); ?>><?php _e('Nome (A-Z)', 'bireme'); ?></option>
            <option value="name_desc" <?php selected($display_order, 'name_desc'); ?>><?php _e('Nome (Z-A)', 'bireme'); ?></option>
            <option value="id_asc" <?php selected($display_order, 'id_asc'); ?>><?php _e('Ordem de criação (Antigas primeiro)', 'bireme'); ?></option>
            <option value="id_desc" <?php selected($display_order, 'id_desc'); ?>><?php _e('Ordem de criação (Novas primeiro)', 'bireme'); ?></option>
            <option value="count_desc" <?php selected($display_order, 'count_desc'); ?>><?php _e('Mais perguntas primeiro', 'bireme'); ?></option>
            <option value="count_asc" <?php selected($display_order, 'count_asc'); ?>><?php _e('Menos perguntas primeiro', 'bireme'); ?></option>
          </select>
        </div>
        ...
        <div class="field">
          <label>
            <input type="checkbox" 
                   name="<?php echo BIREME_LILACS_CP_META_FAQ_SHOW_EMPTY; ?>" 
                   value="1" 
                   <?php checked($show_empty, '1'); ?>>
            <?php _e('Mostrar categorias vazias (sem perguntas publicadas)', 'bireme'); ?>
          </label>
        </div>
      </div>
    </div>
  ...
  // Função para controlar checkboxes de categorias de FAQ
  function toggleAllCategories(checkbox) {
    const checkboxes = document.querySelectorAll('input[name="<?php echo BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES; ?>[]"]');
    checkboxes.forEach(cb => cb.checked = checkbox.checked);
  }

  // Atualizar o "Selecionar todas" baseado no estado atual
  document.addEventListener('DOMContentLoaded', function() {
    const allCheckboxes = document.querySelectorAll('input[name="<?php echo BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES; ?>[]"]');
...
add_action('save_post_page', function($post_id){
  ...
  // Salvar configurações de FAQ
  $selected_categories = isset($_POST[BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES]) 
    ? array_map('intval', $_POST[BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES]) 
    : [];
  update_post_meta($post_id, BIREME_LILACS_CP_META_FAQ_SELECTED_CATEGORIES, $selected_categories);

  $display_order = isset($_POST[BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER]) 
    ? sanitize_text_field($_POST[BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER]) 
    : 'name_asc';
  update_post_meta($post_id, BIREME_LILACS_CP_META_FAQ_DISPLAY_ORDER, $display_order);

  $show_empty = isset($_POST[BIREME_LILACS_CP_META_FAQ_SHOW_EMPTY]) ? '1' : '0';
  update_post_meta($post_id, BIREME_LILACS_CP_META_FAQ_SHOW_EMPTY, $show_empty);
